    ...
    <div className="w-full h-screen bg-slate-900">
      ...
      <div className="w-full h-96 bg-white flex flex-col px-4 py-2">
        ...
        {cartItem?.length === 0 ? (
          <div className="w-full h-full  overflow-auto px-4 py-2 flex items-center justify-center flex-col">
            ...
            <Link
              ...
              className="w-40 h-12 bg-orange-600 text-xl font-bold text-white flex items-center justify-center text-center rounded-xl"
            >
          ...
          <div className="w-full h-full  overflow-auto px-4 py-2">
            {cartItem?.map((item) => {
              return <CartCard item={item} key={item._id} userID={userID} reupdate={getLatestCartData} />;
            })}
          </div>
        )}
        ...
        <div className="w-full p-2 flex items-end flex-col justify-center">
          ...
          <button className="text-lg font-semibold bg-orange-600 text-white uppercase px-4  py-2 cursor-pointer border-2 border-orange-600 rounded-xl hover:bg-transparent hover:text-black duration-700 transition">
            CheckOut
          </button>
        </div>
      </div>
